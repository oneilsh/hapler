...
public class IOHandler {
	 ...
	 * 1.70 : Fixed a bug where sequences with ~'s in TIGR alignment files were not being properly split.
...
"gaps (that are not, by default, allowed.)";
	...
	public void execAndPrintSummaryStats() throws Exception {
		...
		for(int i = 0; i < alignmentList.size(); i++) {
			...
			String useTr = (String) options.valueOf("pos-info-tr-reduction");
			HaplotyperCompat hapler = null;
			if(useTr.compareTo("false") == 0)
				hapler = new HaplotyperCompat(alignment, useEpsilon, numReps, maxReps);
			else
				hapler = new HaplotyperTR(alignment, useEpsilon, numReps, maxReps);
	...
	public MultipleAlignment constructSingleAlignmentFromFasta(String fastaFile, String name) throws Exception {
		...
		ArrayList<MultipleAlignment> alignmentList = new FastaMultipleAlignmentParser().openFile(fastaFile, "true");
	...
	public void printSummaryInfo(HaplotyperCompat hapler, MultipleAlignment alignment, MultipleAlignment groundTruthAlignment, boolean humanReadable) {
		System.err.println("Printing summary info...");

		HashSet<SNP> snpList = alignment.getSNPs();
	...
	public int parseOptions(String[] argv) {
		...
		optionParser.accepts("allow-gaps").withOptionalArg().defaultsTo("false").describedAs("One of false, split, true. The default option, false, ensures that no gaps (unknown bases, such as those between mate pairs) are allowed. If sequences do have haps, you may either split sequences at gaps into seperate sequences (e.g., ignoring mate-pair information), or allow gaps in sequences. If gaps area allowed, the algorithm _may_ produce incorrect results, in the form of internally inconsistent haplotypes. These will be marked, in this case.");
	...
	private ArrayList<MultipleAlignment> parseAlignments(OptionSet options) throws Exception {
		...
		ArrayList<MultipleAlignment> alignmentList = alignmentParser.openFile(input, allowGaps);
