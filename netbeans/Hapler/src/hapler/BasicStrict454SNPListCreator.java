 ...
 * @author soneil
 */
public class BasicStrict454SNPListCreator extends AbstractSNPListCreator {




	public BasicStrict454SNPListCreator() {
		// Empty constructor
	}
	...
	private boolean baseInAlignmentChunk454Variant(String[] seqs, int pos) {
		...
		int numNonGaps = 0;
		int numBaseTypes = 0;
		int majorityBaseCount = 0;
		int minorityBaseCount = 0;
		...
		//for(int i = 0; i < baseToHomoRunsKeySet.length; i++) {
			...
			else if(base == '~') {} // do nothing
			else {
				numNonGaps = numNonGaps + baseCounts.get(base);
				if(majorityBaseCount == 0) { // first we set the majority, then we move the majority down into the majority whenever we find a bigger one
					majorityBaseCount = baseCounts.get(base);
				}
				else {
					if(baseCounts.get(base) > majorityBaseCount) { // move the majority down
						minorityBaseCount = majorityBaseCount;
						majorityBaseCount = baseCounts.get(base);
					}
					else { // check to see if we should replace the minority
						if(baseCounts.get(base) > minorityBaseCount) {
							minorityBaseCount = baseCounts.get(base);
						}
					}
				}
			}
		...
		if(numBaseTypes > 1 && numNonGaps > numGaps && !allBaseTypesHomoRuns && minorityBaseCount >= 2) return true;
